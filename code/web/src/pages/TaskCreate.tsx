...
export default function TaskCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (bodyType !== 'none') payload.content_type = bodyType
    ...
    <div className="pb-10">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/tasks')} className="flex items-center justify-center w-8 h-8 rounded-lg border border-stone-200 text-gray-400 hover:text-gray-900 hover:border-stone-300 transition-colors cursor-pointer">
            <ArrowLeft className="h-4 w-4" />
          </button>
          <div>
            <h2 className="text-2xl font-semibold text-gray-900">创建任务</h2>
            <p className="text-xs text-gray-400">配置定时任务的调度策略与执行参数</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => navigate('/tasks')} className="bg-white border border-stone-200 text-gray-600 rounded-md px-4 py-2 text-sm hover:bg-stone-50 transition-colors duration-150 cursor-pointer">
            取消
          </button>
        ...
        <div className="grid grid-cols-2 gap-5 mb-5">
          ...
          <div className="border border-stone-200 rounded-lg bg-white p-5">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="例：定时同步用户数据" className={inputCls} />
              </div>
          ...
          <div className="border border-stone-200 rounded-lg bg-white p-5">
            ...
            <div className="space-y-3">
              ...
              {scheduleType === 'cron' && (
                <div>
                  <label className={labelCls}>Cron 表达式（6 位）</label>
                  <input type="text" value={form.cron_expr} onChange={(e) => setForm({ ...form, cron_expr: e.target.value })} placeholder="0 */5 * * * *" className={inputCls + " font-mono"} />
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 items-end">
              ...
              <div className="flex-1">
                ...
                <input type="url" required value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} placeholder="https://example.com/webhook" className={inputCls} />
              </div>
            </div>
            ...
            <div className="relative">
              ...
              >
                {selectedCredentials.length === 0 && !credLoading && (
            ...
            <Accordion title="请求体" defaultOpen={false}>
              <div className="grid grid-cols-[200px_1fr] gap-4 items-start">
                ...
                <div className="min-h-0">
                  {bodyType === 'json' && (
                    <textarea value={jsonBody} onChange={(e) => setJsonBody(e.target.value)} placeholder='{"key": "value"}' rows={8} className={inputCls + " font-mono !leading-relaxed resize-y"} />
                  )}
                  {bodyType === 'form' && (
                    <div className="space-y-2">
                      {formFields.map((f, i) => (
